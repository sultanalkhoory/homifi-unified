...
/**
 * PerfectHomeKey Section - Real Door Handle + Proper iPhone Frame
 * 
 * IMPROVEMENTS:
 * 1. Actual door handle mechanism (rotates down like real handle)
 * 2. Uses proper IPhoneFrame component with HomeKey content
 */
        ...
        >
          <div className="mx-auto w-full max-w-4xl">
            ...
            <div className="relative aspect-[16/10] bg-gradient-to-br from-gray-50 to-gray-100 rounded-3xl overflow-hidden shadow-2xl">
              ...
              >
                {/* Use the proper IPhoneFrame structure - VERTICAL orientation - Responsive sizing */}
                <div className="relative w-[160px] h-[320px] sm:w-[200px] sm:h-[400px] md:w-[240px] md:h-[480px]">
                  <div className="relative w-full h-full bg-black rounded-[35px] sm:rounded-[40px] md:rounded-[45px] p-1.5 sm:p-2 shadow-[0_0_0_2px_#1a1a1a,0_0_60px_rgba(0,0,0,0.4)]">
                    <div className="relative w-full h-full bg-black rounded-[28px] sm:rounded-[32px] md:rounded-[37px] overflow-hidden">
                      
                      {/* HomeKey Card Content */}
                      <div className="absolute inset-0 bg-gradient-to-b from-gray-900 to-black">
                        
                        {/* Dynamic Island at top */}
                        <div className="absolute top-1.5 sm:top-2 left-1/2 -translate-x-1/2 z-40">
                          <motion.div
                            animate={{
                              width: lockState === 'locked' ? '50px' 
                                : lockState === 'unlocking' ? '80px' 
                                : '100px',
                              height: lockState === 'locked' ? '14px' 
                          ...
                          >
                            ...
                            {lockState === 'locked' && (
                              <div className="flex items-center justify-center h-full relative w-full">
                                <div className="absolute left-2 w-1 h-1 bg-gray-900 rounded-full" />
                                <div className="absolute right-2 w-2 h-0.5 bg-gray-900 rounded-full" />
                              </div>
                            )}
                            ...
                            <AnimatePresence mode="wait">
                              {lockState !== 'locked' && (
                                <motion.div
                                  ...
                                  className="flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2"
                                >
                                  {/* Icon */}
                                  {lockState === 'unlocking' ? (
                                    <motion.svg 
                                      className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 text-white flex-shrink-0" 
                                      fill="none" 
                                      stroke="currentColor" 
                                      viewBox="0 0 24 24" 
                                      strokeWidth={2}
                                  ...
                                  <motion.span
                                    ...
                                    className="text-white text-[9px] sm:text-[10px] md:text-xs font-medium whitespace-nowrap"
                                  >
                        ...
                        {/* Status bar - 9:41 */}
                        <div className="absolute top-1.5 sm:top-2 left-3 sm:left-4 text-white text-[10px] sm:text-xs md:text-sm font-medium z-30 drop-shadow-sm">
                          9:41
                        </div>

                        {/* HomeKey Card - Centered and visible */}
                        <motion.div 
                          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[75%] sm:w-[80%]"
                          animate={{
                        ...
                        >
                          <div className="bg-white/10 backdrop-blur-xl rounded-xl sm:rounded-2xl p-3 sm:p-4 md:p-5 border border-white/20">
                            ...
                            <div className="flex justify-center mb-2 sm:mb-3">
                              <motion.div 
                                className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-full bg-orange-500 flex items-center justify-center relative"
                                animate={{
                            ...
                            <div className="text-center">
                              <p className="text-white text-xs sm:text-sm md:text-sm font-semibold mb-0.5 sm:mb-1">Front Door</p>
                              <p className="text-white/70 text-[10px] sm:text-xs md:text-xs">
                                {lockState === 'unlocking' ? 'Unlocking...' : 'Hold Near Reader'}
                              </p>
                            </div>

                            {/* NFC waves - animated */}
                            <div className="flex justify-center mt-2 sm:mt-3 relative h-4 sm:h-5">
                              {/* NFC icon base */}
                              <svg className="w-8 h-4 sm:w-10 sm:h-5 text-white/50" fill="currentColor" viewBox="0 0 24 24">
                              ...
                              {lockState === 'unlocking' && (
                                <>
                                  ...
                                  >
                                    <div className="w-6 h-6 sm:w-8 sm:h-8 border-2 border-blue-400 rounded-full" />
                                  </motion.div>
                                  <motion.div
                                    className="absolute inset-0 flex items-center justify-center"
                                    initial={{ scale: 0.8, opacity: 0 }}
                                  ...
                                  >
                                    <div className="w-9 h-9 sm:w-12 sm:h-12 border-2 border-blue-400 rounded-full" />
                                  </motion.div>
                                </>
                              )}
                            </div>
                      ...
                      {/* Subtle screen glare */}
                      <div
                        className="absolute inset-0 pointer-events-none rounded-[28px] sm:rounded-[32px] md:rounded-[37px]"
                        style={{
                          background:
                  ...
                  <motion.div
                    className="absolute -left-3 sm:-left-4 md:-left-5 top-0 bottom-0 w-6 sm:w-8 md:w-10"
                    animate={{
                      opacity: lockState === 'locked' ? 0.1 : 0.2,
                      x: lockState === 'locked' ? 0 : -10
                    }}
                    style={{
