    ...
    case 'plan':
      return (
        <motion.div
          ...
          className="absolute inset-0 bg-gradient-to-br from-blue-50 to-indigo-50 p-6"
        >
          {/* Floor plan visualization */}
          <div className="h-full flex flex-col gap-4">
            <div className="text-center pt-12">
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Your Home Layout</h3>
              <p className="text-xs text-gray-600">Custom zone mapping</p>
            </div>
            
            {/* Simple zone grid */}
            <div className="flex-1 grid grid-cols-2 gap-3 p-4">
              {['Living Room', 'Kitchen', 'Bedroom', 'Office'].map((zone, i) => (
                <motion.div
                  key={zone}
                  initial={{ scale: 0.8, opacity: 0 }}
                  animate={{ scale: 1, opacity: 1 }}
                  transition={{ delay: i * 0.1 }}
                  className="rounded-2xl bg-white/80 backdrop-blur-sm p-4 shadow-sm border border-gray-200"
                >
