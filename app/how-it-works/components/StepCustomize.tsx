...
export default function StepCustomize({ 
  ...
  return (
    <motion.div
      ...
      className={`${fullScreen ? 'rounded-3xl shadow-2xl' : 'absolute inset-0'} bg-gradient-to-b from-white via-slate-50/50 to-white p-8 md:p-12 overflow-hidden relative`}
    >
          ...
          <div className="w-full max-w-2xl">
            ...
            <AnimatePresence mode="wait">
              ...
              {buildStep === 1 && (
                ...
                >
                  <h4 className="text-3xl font-semibold text-slate-900 mb-8 text-center tracking-tight">
                    Name your scene
                  </h4>
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-7 py-5 text-xl bg-slate-50/80 border-2 border-slate-200/50 rounded-[20px] focus:border-slate-900 focus:bg-white focus:outline-none transition-all duration-300 placeholder:text-slate-400"
                    />
                    <AnimatePresence>
                      {sceneName && (
                        <motion.div
              ...
              {buildStep === 2 && (
                <motion.div
                  ...
                  style={{ boxShadow: '0 20px 60px rgba(0,0,0,0.08)' }}
                >
                  <h4 className="text-3xl font-semibold text-slate-900 mb-3 text-center tracking-tight">
                    Select devices
                  </h4>
                  <p className="text-slate-500 text-center mb-10 text-lg font-light">Tap to include in "{sceneName}"</p>
                    ...
                    {availableDevices.map((device, index) => {
                      ...
                      return (
                        <motion.button
                          ...
                          whileTap={{ scale: 0.98 }}
                          className={`relative p-7 rounded-[24px] transition-all duration-300 ${
                            isSelected
                          ...
                          <AnimatePresence>
                            {isSelected && (
                              <motion.div
                                ...
                                className="absolute top-4 right-4 w-7 h-7 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center"
                              >
              ...
              {buildStep === 3 && (
                ...
                >
                  ...
                  <p className="text-slate-500 text-center mb-10 text-lg font-light">Set what each device should do</p>
                    ...
                    {selectedDevices.map((deviceId, index) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="p-6 bg-gradient-to-br from-slate-50/80 to-slate-100/50 rounded-[20px] border border-slate-100/50"
                        >
                          <div className="flex items-center gap-4 mb-5">
                            <div className={`w-12 h-12 rounded-[14px] bg-gradient-to-br ${device.color} flex items-center justify-center text-white shadow-sm`}>
                              {device.icon}
                            </div>
                            <div className="flex-1">
                              <div className="font-semibold text-slate-900 text-lg">{device.name}</div>
                              <div className="text-sm text-slate-500">{device.actionLabel}</div>
                            </div>
                          </div>
                            ...
                            <div className="flex gap-3">
                              {(device.id === 'curtains' ? ['Open', 'Closed'] : ['On', 'Off']).map(option => (
                                <motion.button
                                  ...
                                  whileTap={{ scale: 0.96 }}
                                  className={`flex-1 py-3.5 rounded-[14px] text-base font-semibold transition-all duration-300 ${
              ...
              {buildStep === 4 && (
                <motion.div
                  ...
                  className="bg-gradient-to-br from-emerald-500 via-green-500 to-emerald-600 rounded-[32px] p-10 md:p-14 shadow-2xl text-white text-center relative overflow-hidden"
                >
                  {/* Glass overlay */}
                  ...
                  <div className="relative">
                    <motion.div
                      ...
                      className="w-24 h-24 mx-auto mb-8 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center shadow-xl"
                    >
                    ...
                    <motion.div
                      ...
                      className="bg-white/15 backdrop-blur-md rounded-[20px] p-6 border border-white/10"
                    >
                      <p className="text-sm text-white/80 mb-4 font-medium">Controlling {selectedDevices.length} devices</p>
                        ...
                        {selectedDevices.map((deviceId, i) => {
                          ...
                          return (
                            <motion.div
                              ...
                              className="px-4 py-2 bg-white/20 backdrop-blur-sm rounded-full text-sm font-medium border border-white/10"
                            >
                              {device.name}
                            </motion.div>
                          );
