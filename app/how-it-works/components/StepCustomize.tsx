...
export default function StepCustomize({ 
  ...
  return (
    ...
    >
      ...
      {/* Flex container fills available space and centers content */}
                    ...
                    {devices.map((device, index) => {
                      ...
                      return (
                        <motion.button
                          ...
                          className={`relative p-5 md:p-7 rounded-2xl md:rounded-[24px] transition-all duration-500 text-left ${
                            isSelected
                    ...
                    {selectedDevices.map((deviceId, index) => {
                      ...
                      return (
                        <motion.div
                          ...
                          className="bg-slate-50/80 rounded-2xl md:rounded-[20px] p-4 md:p-5"
                        >
                          <div className="flex items-center justify-between mb-3 md:mb-4">
                            <div className="flex items-center gap-2 md:gap-3">
                              <DeviceIcon className="text-slate-600">{device.icon}</DeviceIcon>
                              <span className="text-sm md:text-base font-semibold text-slate-900">{device.name}</span>
                            </div>
                            {device.actionType === 'slider' && (
                              <span className="text-sm md:text-base font-bold text-slate-900">{value}%</span>
                            )}
                          </div>
                          ...
                          {device.actionType === 'slider' && (
                            <input
                              ...
                              className="w-full h-2 md:h-2.5 bg-slate-200 rounded-full appearance-none cursor-pointer slider"
                              style={{
                            ...
                            <div className="flex gap-2 md:gap-3">
                              {device.options.map((option) => (
                                <motion.button
                                  ...
                                  className={`flex-1 py-2.5 md:py-3 rounded-xl md:rounded-[14px] text-sm md:text-base font-semibold transition-all duration-300 ${
